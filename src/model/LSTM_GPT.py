...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_ids, attention_mask, sentiment_label
...
def plot_accuracy(epochs_range, train_accuracy_history, val_accuracy_history, parameters):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(os.path.join(parameters['result_path'], 'LSTM_GPT_Accuracy.png'))
def plot_loss(epochs_range, train_loss_history, val_loss_history, parameters):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(parameters['result_path'], 'LSTM_GPT_Loss.png'))

def train_validate_test_LSTM_GPT(tokenized_data, vocab_size,logger, parameters):
    ...
    for epoch in tqdm(range(num_epochs), desc=f'Epochs'):
        ...
        with torch.no_grad():
            for val_batch in tqdm(val_loader, desc=f'Validation', leave=False):
                val_input_ids, val_attention_mask, val_labels = [tensor.to(device) for tensor in val_batch]
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(parameters['model_path'],f'best_model_epoch_{epoch+1}_GPT.pth'))
    ...
    with torch.no_grad():
        for test_batch in tqdm(test_loader, desc=f'Testing'):
            test_input_ids, test_attention_mask,  test_labels = [tensor.to(device) for tensor in test_batch]
